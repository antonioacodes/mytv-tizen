(function () {
  ...
  function bindSidebar() { document.querySelectorAll('[data-nav]').forEach(function(button){ button.addEventListener('click',function(){ var page=button.getAttribute('data-nav'); if(page==='home') { renderHome(); return; } if(page==='search') { renderSearch(); return; } if(page==='settings') { renderSettings(); return; } renderPlaceholder(page); }); }); }
  ...
  function renderMediaDetails(mediaId,isTv,season) {
    ...
    function show(payload) {
      ...
      app.innerHTML='<section class="media-details"><div class="details-backdrop" style="background-image:url(\''+cleanUrl(backdrop)+'\')"></div><div class="details-gradient"></div><button class="details-back" id="details-back">← <span>VOLTAR</span></button><main class="details-content"><section class="details-hero">'+
        ...
        '<section class="details-body"><p class="details-overview">'+escapeHtml(item.overview || 'Sinopse indisponível.')+'</p>'+ageInfo(item.certification)+'<div class="details-actions"><button class="primary-action" id="detail-play">▶ ASSISTIR</button><button class="secondary-action">♡ FAVORITAR</button></div>'+
      ...
      document.getElementById('detail-play').addEventListener('click',function(){ var first=episodes[0] || null; openVod({mediaId:mediaId,imdbId:item.imdb_id || '',title:title,isTv:isTv,season:first ? Number(first.season_number || season) : Number(season),episode:first ? Number(first.episode_number) : 1,episodes:episodes,backdrop:backdrop,certification:item.certification || ''}); });
  ...
  function renderVodPlayer(data,sources) {
    var sourceIndex=0, overlayVisible=true, menuVisible=false, subtitles=[], subtitleIndex=-1, aspectIndex=0, ratingShown=false, nextShown=false, source=sources[0];
    ...
    function loadSource(index,position) { sourceIndex=index; source=sources[index]; setBuffer(true); document.getElementById('vod-source-state').textContent='Carregando'; video.src=source.url; video.load(); video.onloadedmetadata=function(){if(position) video.currentTime=Math.min(position,video.duration||position);}; video.play().catch(function(){}); }
    ...
    video.addEventListener('timeupdate',function(){var duration=video.duration||0;document.getElementById('vod-current').textContent=formatTime(video.currentTime);document.getElementById('vod-total').textContent=formatTime(duration);document.getElementById('vod-seek').value=duration?Math.round(video.currentTime/duration*1000):0;var upcoming=nextEpisode();if(data.isTv && upcoming && duration && duration-video.currentTime<=75 && !nextShown){nextShown=true;next.classList.add('is-visible');next.focus();}});
    video.addEventListener('ended',function(){var upcoming=nextEpisode();if(upcoming){nextShown=true;next.classList.add('is-visible');next.focus();}else returnFromVod();});
    ...
    bottom.querySelectorAll('[data-vod-menu]').forEach(function(button){button.addEventListener('click',function(){showMenu(button.getAttribute('data-vod-menu'));});}); next.addEventListener('click',function(){var upcoming=nextEpisode();if(upcoming)openVod({mediaId:data.mediaId,imdbId:data.imdbId,title:data.title,isTv:true,season:Number(upcoming.season_number||upcoming.seasonNumber),episode:Number(upcoming.episode_number||upcoming.episodeNumber),episodes:data.episodes,backdrop:data.backdrop,certification:data.certification});});
