(function () {
  ...
  function bindSidebar() { document.querySelectorAll('[data-nav]').forEach(function(button){ button.addEventListener('click',function(){ var page=button.getAttribute('data-nav'); if(page==='home') { renderHome(); return; } if(page==='search') { renderSearch(); return; } if(page==='favorites') { renderFavorites(); return; } if(page==='sports') { renderSports(); return; } if(page==='live') { renderLiveTv(); return; } if(page==='settings') { renderSettings(); return; } renderPlaceholder(page); }); }); if(document.activeElement===document.body){var active=document.querySelector('.side-menu-item.is-active')||document.querySelector('.side-menu-item');if(active)active.focus();} }
  ...
  function livePlaybackUrl(resolved) { return (resolved && ((resolved.delivery_mode && resolved.delivery_mode!=='direct' ? resolved.proxy_url : resolved.url) || resolved.proxy_url || resolved.url)) || ''; }
  ...
  function renderLiveTv() {
    ...
    app.innerHTML='<section class="live-page">'+sidebarMarkup('live')+'<main class="live-content"><div class="live-page-heading"><div><h1>TV ao Vivo</h1><p>Canais e programação em tempo real</p></div><button class="sports-refresh" id="live-refresh">↻ ATUALIZAR</button></div><div class="live-loading"><span class="loader"></span>Carregando canais…</div></main></section>';bindSidebar();
