(function () {
  ...
  function sportCard(match) {
    ...
    return '<button class="sports-card" data-sport-match="'+sportEscape(match.id)+'" style="background-image:linear-gradient(180deg,rgba(3,8,20,.08),rgba(3,8,20,.94)),url(\''+cleanUrl(bg)+'\')"><div class="sports-card-top">'+(live?'<span class="sports-live-badge">AO VIVO</span>':'')+(reminded?'<i class="sports-reminder-badge" aria-label="Lembrete ativo">🔔</i>':'')+'</div><div class="sports-card-teams"><div><img src="'+attrUrl(match.homeBadgeUrl || '')+'" alt=""><b>'+sportEscape(match.homeTeam)+'</b></div><strong>'+sportEscape(score || '—')+'</strong><div><img src="'+attrUrl(match.awayBadgeUrl || '')+'" alt=""><b>'+sportEscape(match.awayTeam)+'</b></div></div><div class="sports-card-bottom"><span>'+sportEscape(sportLongDate(match))+'</span></div></button>';
  }
  ...
  function renderSportsOverlay(match) {
    ...
    function paint(sectionData,loading) {
      var retainedPlayer=document.querySelector('.sports-video'), broadcast=broadcasts[sourceIndex], body=tab===0?sportOverview(match,broadcast):(loading?'<div class="sports-section-message"><div class="loader"></div>Carregando informações da partida…</div>':tab===1?sportsLineups(sectionData,match):tab===2?sportsStatistics(sectionData):sportsStandings(sectionData));
      if(retainedPlayer && retainedPlayer.parentNode) retainedPlayer.parentNode.removeChild(retainedPlayer);
      app.innerHTML='<section class="sports-watch '+(fullscreen?'is-fullscreen':'')+'">'+sportPlayerMarkup(match)+'<section class="sports-watch-panel" style="background-image:linear-gradient(90deg,rgba(3,12,24,.98),rgba(3,12,24,.90)),url(\''+cleanUrl(match.backgroundUrl || SPORTS_BACKGROUND_URL)+'\')"><div class="sports-watch-title"><div><h1>'+sportEscape(match.leagueName)+'</h1><p>'+sportEscape(sportLongDate(match))+'</p></div><div class="sports-watch-actions"><button id="sport-back" aria-label="Voltar">←</button><button id="sport-reminder" aria-label="Lembrete">🔔</button><button id="sport-source" aria-label="Trocar canal" '+(broadcasts.length>1?'':'disabled')+'>↻</button><button id="sport-fullscreen" aria-label="Tela cheia">⛶</button></div></div><div class="sports-tabs">'+['RESUMO','ESCALAÇÕES','ESTATÍSTICAS'].map(function(name,index){return '<button data-sport-tab="'+index+'" class="'+(tab===index?'selected':'')+'">'+name+'</button>';}).join('')+'</div><div class="sports-watch-data">'+body+'</div></section></section>';
