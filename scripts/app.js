(function () {
  ...
  function activateLiveStream(video,channel,index,tuneToken) {
    var streams=liveStreamList(channel),stream=streams[index];
    if(!video||!stream||stream.is_working===false||!stream.url) return Promise.reject(new Error('Nenhuma fonte online disponível'));
    liveTvState.sourceIndex=index;
    if(tuneToken && tuneToken!==liveTvState.tuneToken) return Promise.reject(new Error('Sintonia substituída'));
    var url=decodePlayableUrl(livePlaybackUrl(stream));
    if(!url) return Promise.reject(new Error('Canal indisponível'));
    video.pause();video.removeAttribute('src');video.load();video.muted=false;video.volume=1;video.src=url;video.load();liveTvState.videoChannelId=channel.id;liveTvState.traffic='12 KB/s';
  ...
  function tuneLiveChannel(channel,video,onDone) {
    ...
    function attempt(){if(tuneToken!==liveTvState.tuneToken)return;if(index>=streams.length){liveTvState.loading=false;if(onDone)onDone('Não foi possível sintonizar este canal agora.');return;}activateLiveStream(video,channel,index,tuneToken).then(function(){if(tuneToken!==liveTvState.tuneToken)return;liveTvState.loading=false;if(onDone)onDone();}).catch(function(){if(tuneToken!==liveTvState.tuneToken)return;index+=1;attempt();});}
    attempt();
  }
